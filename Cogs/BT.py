...
class BT(commands.Cog):
    ...
    async def s_release(self, ctx: commands.Context, errorid: int):
        ...
        await self.bot.http.delete_message(666875639164436521, data[1], reason=f"Bug Fixed (#{errorid})")
    ...
    async def release(self, ctx: commands.Context, errorid: int):
        ...
        emb.timestamp = datetime.datetime.utcnow()
        emb.set_footer(text="Issue released at")
        await self.bot.http.delete_message(666878516469563402, data[1], reason=f"Bug Fixed (#{errorid})")
