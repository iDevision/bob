...
class BT(commands.Cog):
    ...
    async def s_release(self, ctx: commands.Context, errorid: int):
        ...
        await (await self.bot.get_channel(666875639164436521).fetch_message(data[1])).delete(reason=f"Bug Fixed (#{errorid})")
    ...
    async def release(self, ctx: commands.Context, errorid: int):
        ...
        emb.timestamp = datetime.datetime.utcnow()
        emb.set_footer(text="Issue released at")
        await (await self.bot.get_channel(666878516469563402).fetch_message(data[1])).delete(reason=f"Bug Fixed (#{errorid})")
